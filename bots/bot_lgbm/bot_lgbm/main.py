...
def process(input_directory=None, output_directory=None):
    ...
    # Process
    clf = LGBMClassifier(n_estimators=500, colsample_bytree=0.3)
    trained_model = clf.fit(X, Y)

    # Write locally (artifacts + results - output folder)
    pickle_file_name = os.path.join(output_directory, 'lgbm_classifier_model.pkl')
...
def transform(input_directory=None, output_directory=None):
    ...
    trained_model_filename = os.path.join(input_directory, 'lgbm_classifier_model.pkl')
