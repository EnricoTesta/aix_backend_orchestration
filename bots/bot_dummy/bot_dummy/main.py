...
def process(input_directory=None, output_directory=None):
    ...
    d = data_dict['d_train']
    Y = d['TARGET_VARIABLE'].to_numpy(copy=True)
    X = d.drop(columns=['TARGET_VARIABLE', 'OBS_DATE', 'Ticker']).fillna(value=0).to_numpy()
    ...
    pickle_file_name = os.path.join(output_directory, 'dummy_classifier_model.pkl')
...
def transform(input_directory=None, output_directory=None):
    ...
    trained_model_filename = os.path.join(input_directory, 'dummy_classifier_model.pkl')
    with open(trained_model_filename, 'rb') as f:
        trained_model = load(f)

    # Predict
    tmp = DataFrame(trained_model.predict_proba(d.to_numpy()))
